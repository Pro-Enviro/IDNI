...
import {Data} from "../../../../_helpers/models/data";
import {AdminService} from "../../../../_services/admin.service";
import {CfarService} from "../../../../_services/cfar.service";
...
export class EnvirotrackReportFieldsComponent {
    ...
    if (id) {
      this.admin.fnGetCompanyDetails(id, [
        ...
        next: (res: any) => {
          ...
          if (res?.fuel_data[0]?.all_other_fields) {
            const parsedData = JSON.parse(res.fuel_data[0].all_other_fields)
            this.data = parsedData
            this.organiseData(parsedData)
          } else {
            this.data = []
          }
        },
  ...
  splitByMonth = (data: any) => {
    ...
    dataToMap.map((fuelType: any) => {
      ...
      rows.map((row: any) => {
        ...
        if (uom !== 'kWh') {
          const newValue = this.convertToKwh(fuelType, uom, value)
          value = newValue
        }
  ...
  getCFARData = (id: number) => {
    this.cfar.getCfarReport().subscribe({
      next: (res: any) => {
        let data: any = [];
        const rawData = res.data.companies.map((x: any) => x.companies_id)
        rawData.forEach((x: any) => x.cfar.forEach((row: any) => {
          data.push(row.cfar_data_id)
        }))
        ...
        this.fieldFilters = new Set<string>(this.fieldFilters)
        // Removing [null] from Set if no field filters
        if (!this.fieldFilters.length) this.fieldFilters = []

        // Sort data by dates
        this.data = data;
        this.months = data.map((x: any) => x.endDate)
